...
public class QueryResponse {
	...
	public QueryResponse(byte[] data, boolean fullstat) {
		...
		if (!fullstat) {
			motd = new String(
					ByteUtils.subarray(temp[0], 1, temp[0].length - 1));
			...
			hostname = new String(ByteUtils.subarray(temp[5], 2,
					temp[5].length - 1));
		} else // full stat
		{
	 ...
	 * players' usernames. Note that this will return null for basic status
	 * requests.
	 * 
